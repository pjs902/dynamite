...
def test_valid_fiducial_passes():
    clipped, violations = validate_parset(
        _par(), FAKE_BOUNDS, qobs=QOBS, u_fixed=U_FIXED
    )
    assert violations == []
...
def test_out_of_bounds_clips_not_rejects():
    clipped, violations = validate_parset(
        _par(bh=9.0), FAKE_BOUNDS, qobs=QOBS, u_fixed=U_FIXED
    )
    assert violations == []
...
def test_q_above_qobs_times_u_is_violation():
    ...
    _, violations = validate_parset(_par(q=0.74), loose, qobs=QOBS, u_fixed=U_FIXED)
...
def test_capped_bounds_repair_geometry_silently():
    ...
    # violation surfaces
    clipped, violations = validate_parset(
        _par(q=0.74), FAKE_BOUNDS, qobs=QOBS, u_fixed=U_FIXED
    )
    assert violations == []
...
def test_p_less_than_q_is_violation():
    _, violations = validate_parset(
        _par(q=0.8, p=0.7), FAKE_BOUNDS, qobs=QOBS, u_fixed=U_FIXED
    )
...
def test_nonfinite_value_rejected():
    _, violations = validate_parset(
        _par(ml=float("nan")), FAKE_BOUNDS, qobs=QOBS, u_fixed=U_FIXED
    )
...
def test_u_free_uses_parset_u_not_fixed():
    ...
    clipped, violations = validate_parset(
        _par(q=0.68, u=0.95), FAKE_BOUNDS, qobs=QOBS, u_fixed=None
    )
    assert violations == []
